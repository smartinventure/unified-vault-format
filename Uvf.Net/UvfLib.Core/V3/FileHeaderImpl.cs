...
public sealed class FileHeaderImpl : FileHeader
{
    ...
    public const int UVF_MAGIC_BYTES_LENGTH = 4; // 'u', 'v', 'f', 0x00
    public const int UVF_GENERAL_HEADERS_LEN = UVF_MAGIC_BYTES_LENGTH + sizeof(int); // Magic bytes + seed ID
    public const int NONCE_LEN = 12; // 96 bits
    public const int CONTENT_KEY_LEN = 32; // 256 bits
    public const int TAG_LEN = 16; // 128 bits
    public const int NONCE_POS = UVF_GENERAL_HEADERS_LEN;
    public const int CONTENT_KEY_POS = NONCE_POS + NONCE_LEN;
    public const int SIZE = CONTENT_KEY_POS + CONTENT_KEY_LEN + TAG_LEN;
    ...
    public void Destroy()
    {
        if (!_destroyed)
        {
            UvfLib.Core.Common.CryptographicOperations.ZeroMemory(_nonce);
            _contentKey?.Destroy();
            _destroyed = true;
        }
    }
