...
namespace ExampleVaultApp
{
    ...
    public class ChangePwCryptomatorTest
    {
        ...
        private async Task CreateTestVaultAsync()
        {
            ...
            using var vault = await VaultManager.CreateVaultAsync(_testVaultPath, _originalPassword);
        ...
        {
            using var vault = await VaultManager.LoadVaultAsync(_testVaultPath, password);
        ...
        private async Task VerifyPasswordRejectionAsync(string password)
        {
            ...
            catch (Exception ex) when (ex.Message.Contains("password") || ex.Message.Contains("passphrase") || ex.Message.Contains("credential"))
            {
                Console.WriteLine($"✅ Password correctly rejected: {ex.Message}");
            }
        }
        ...
        private async Task ChangeVaultPasswordAsync()
        {
            ...
            try
            {
                ...
                await VaultManager.ChangeVaultPasswordAsync(_testVaultPath, _originalPassword, _newPassword);
        ...
        /// <summary>
        /// Updates the vault.cryptomator file with a new signature based on the new masterkey
        /// </summary>
        private async Task UpdateVaultConfigAsync(byte[] newMasterkeyContent)
        {
            // This method is no longer needed since VaultManager.ChangeVaultPasswordAsync() 
            // handles both masterkey and vault config updates internally
            Console.WriteLine("✅ Vault configuration updated by VaultManager");
        }
